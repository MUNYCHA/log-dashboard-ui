...
const ServerCard = ({ server, darkMode }) => {
  ...
  return (
    <div className={`rounded-xl overflow-hidden border ${
      darkMode ? 'bg-[#252525] border-[#3C4043]' : 'bg-white border-[#E8EAED]'
    }`}>

      {/* Card header */}
      <div className={`px-5 py-4 flex items-center justify-between gap-3 border-b ${divider} ${
        darkMode ? 'bg-[#2C2C2C]' : 'bg-[#F8F9FA]'
      }`}>
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className={`text-[15px] font-bold truncate ${darkMode ? 'text-[#E8EAED]' : 'text-[#202124]'}`}>
              {server.serverName}
            </p>
            <p className={`text-[12px] font-mono ${darkMode ? 'text-[#5F6368]' : 'text-[#9AA0A6]'}`}>
              {server.serverIp} · {collectedAt}
            </p>
          </div>
        </div>
      </div>
